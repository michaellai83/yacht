...
namespace yacht
{
    public partial class Back_DealersAdd : System.Web.UI.Page
    {
        ...
        protected void Country_sub_Click(object sender, EventArgs e)
        {
            ...
            if (Regex.IsMatch(Email.Text.ToString(), "[a-zA-Z0-9_]+@[a-zA-Z0-9._]+"))
            {
            ...
            if (city != "" && saletype != "" && contact != "" && add != "" && tel != "" && email != "")
            {
                if (fax != "")
                {
                    ...
                    connection.Close();
                    Response.Write($"<script>alert('已新增資料');;location.href='Back_DealersAdd.aspx';</script>");
                }
                else
                {
                    SqlConnection connection = new SqlConnection(SqlName);
        ...
        {
            Response.Redirect("Back_DealersCountry.aspx");
        }
    }
}